...
// File names must be alpha-numeric characters (0-9, a-z, A-Z) or "/" underscore
// "_" other ascii characters are stripped out by client, including / generates
// sub-directories
#define DEFAULT_FILENAME                                                       \
  "tft_screenshots/screenshot" // In case none is specified
#define FILE_TYPE "png"        // jpg, bmp, png, tif are valid

// Filename extension
// '#' = add incrementing number, '@' = add timestamp, '%' add millis()
// timestamp,
// '*' = add nothing
// '@' and '%' will generate new unique filenames, so beware of cluttering up
// your hard drive with lots of images! The PC client sketch is set to limit the
// number of saved images to 1000 and will then prompt for a restart.
#define FILE_EXT '@'
...
public:
  ...
  void drawBezier(int32_t x0, int32_t y0, int32_t x1, int32_t y1, int32_t x2,
...
#ifdef ESP32
  // Draw a jpeg stored in an array using the ESP32 native decoder, can crop and
  // scale
  ...
  // Draw a jpeg stored in a file using the ESP32 native decoder, can crop and
  // scale (Tested with SPIFFS file)
  bool drawJpgFile(fs::FS &fs, const char *path, uint16_t x = 0, uint16_t y = 0,
