...
// The class inherits the generic drawing graphics functions from the TFT_eSPI
// class.
...
void TFT_eFEX::drawBezier(int32_t x0, int32_t y0, int32_t x1, int32_t y1,
...
//  p0 x                                                   x p0
//
void TFT_eFEX::drawBezierSegment(int32_t x0, int32_t y0, int32_t x1, int32_t y1,
                                 int32_t x2, int32_t y2, uint16_t color) {
  ...
  if (((x2 >= x1 && x1 >= x0) || (x2 <= x1 && x1 <= x0)) &&
    ...
    Serial.println("Bad coordinate set - non-sequential!");
}

/***************************************************************************************
** Function name:           drawBmp
** Description:             draw a bitmap stored in SPIFFS onto the TFT or in a
*Sprite
***************************************************************************************/
// void TFT_eFEX::drawBmp(const char *filename, int16_t x, int16_t y,
// TFT_eSprite *_spr) {
void TFT_eFEX::drawBmp(String filename, int16_t x, int16_t y,
                       TFT_eSprite *_spr) {

  if ((_spr == nullptr) && ((x >= _tft->width()) || (y >= _tft->height())))
    return;
    ...
    if ((read16(bmpFS) == 1) && (read16(bmpFS) == 24) && (read32(bmpFS) == 0)) {
      ...
      for (row = 0; row < h; row++) {
        ...
        if (_spr == nullptr)
          ...
          _spr->pushImage(x, y--, w, 1, (uint16_t *)lineBuffer);
      }
      ...
      Serial.println("BMP format not recognised.");
  }
  bmpFS.close();
}
...
***************************************************************************************/
void TFT_eFEX::drawProgressBar(uint16_t x, uint16_t y, uint16_t w, uint16_t h,
                               uint8_t percent, uint16_t frameColor,
                               uint16_t barColor) {
  if (percent == 0) {
  ...
  _tft->fillRect(x + margin, y + margin, barWidth * percent / 100.0, barHeight,
                 barColor);
}
...
/***************************************************************************************
** Function name:           luminance
** Description:             return a 16 bit colour from an RGB with reduced
*luminance
***************************************************************************************/
uint16_t TFT_eFEX::luminance(uint8_t red, uint8_t green, uint8_t blue,
                             uint8_t luminance) {
...
** Description:             Draw the string Arabic RTL and update cursor
*coordinates
***************************************************************************************/
...
bool TFT_eFEX::screenServer(void) {
  // With no filename the screenshot will be saved with a default name e.g.
  // tft_screen_#.xxx where # is a number 0-9 and xxx is a file type specified
  // below
...
bool TFT_eFEX::screenServer(String filename) {
  ...
  boolean result =
      serialScreenServer(filename); // Screenshot serial port server
  // boolean result = wifiScreenServer(filename);   // Screenshot WiFi UDP port
  // server (WIP)
...
bool TFT_eFEX::serialScreenServer(String filename) {
  ...
  while (wait) {
    ...
    if (Serial.available() > 0) {
      ...
      // If it is 'S' (start command) then clear the serial buffer for 100ms and
      // stop waiting
      if (cmd == 'S') {
        // Precautionary receive buffer garbage flush for 50ms
        clearTime = millis() + 50;
        ...
        // Send screen size etc using a simple header with delimiters for client
        // checks
  ...
  for (uint32_t y = 0; y < _tft->height(); y++) {
    ...
    for (uint32_t x = 0; x < _tft->width(); x += NPIXELS) {
      ...
      if (NPIXELS > 1)
        _tft->readRect(x, y, NPIXELS, 1, (uint16_t *)color);
      else {
...
bool TFT_eFEX::drawJpg(const uint8_t *jpg_data, size_t jpg_len, uint16_t x,
...
bool TFT_eFEX::drawJpgFile(fs::FS &fs, const char *path, uint16_t x, uint16_t y,
